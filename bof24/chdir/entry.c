...
)
{
    ...
    if (Length) {
        ...
        if (path && path[0] == '\0') { path = NULL; }
    }

    if (path) {
        if (!KERNEL32$SetCurrentDirectoryA(path)) {
            BeaconPrintf(CALLBACK_ERROR, "Failed to change directory: %u", KERNEL32$GetLastError());
            printoutput(TRUE);
            return;
        }
    }
    ...
    char curdir[1024] = {0};
    DWORD len = KERNEL32$GetCurrentDirectoryA(sizeof(curdir)-1, curdir);
    if (len) {
        internal_printf("Current Directory: %s\n", curdir);
    } else {
        BeaconPrintf(CALLBACK_ERROR, "Unable to get current directory: %u", KERNEL32$GetLastError());
    }
    printoutput(TRUE);
};
